<template>
  ...
  async function loadForestAndCarbonData() {
    try {
      forestAndCarbonData = await d3.csv('/public/13_Forest_and_Carbon.csv');
      ...
      drawScatterPlot(mergedData, props.selectedYear);
    } catch (error) {
  ...
  watch(() => props.selectedYear, loadForestAndCarbonData);
  onMounted(loadForestAndCarbonData);
  </script>
